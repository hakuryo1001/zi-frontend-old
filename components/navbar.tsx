...
const NavElement = ({ href, name }: any) => {
    ...
    if (active) return (
        <Link href={href}>
            <a className={styles.navElement} aria-current="page">{name}</a>
        </Link>
    ); else return (
        <Link href={href}>
            <a className={styles.navElement}>{name}</a>
        </Link>
    );
}
            ...
            <div className="flex items-center">
                ...
                <div className="hidden w-full md:block md:w-auto " id="navbar-default">
                    ...
                    <ul className="flex flex-cols place-items-center p-4 mt-4  rounded-lg border border-gray-100 md:flex-row md:space-x-8 md:mt-0 md:text-sm md:font-medium md:border-0 dark:bg-gray-800 md:dark:bg-gray-900 dark:border-gray-700">
                        ...
                        <Link href="https://twitter.com/0xcanto1">
                            <svg className="w-5 h-5 fill-white hover:white" viewBox="0 0 24 24" aria-hidden="true">
